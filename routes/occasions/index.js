...
router.post(
  ...
  [vs.isValidStrLenWithTrim('body', 'name', 3, 50, 'Please enter a valid name'), vs.isYear('body', 'year', 'Please provide a valid Year')],
  async (req, res) => {
    ...
    try {
      const [rows] = await db.insertOccasions(req.body);
...
router.get('/list', async (req, res) => {
  try {
    const [rows] = await db.getOccasions();
    return res.status(200).send(responseGenerator.success('Occasions List', 'Occasions retrieved successfully', rows));
  } catch (e) {
    console.log(e);
    if (e.code === 'ER_DUP_ENTRY') {
      const beUserDuplicateEntry = error.errList.dbError.ERR_INSERT_OCCASION_DUPLICATE_ENTRY;
...
/**
 * @api {get} /occasion/dashboard/summary Occasion list
 * @apiVersion 1.0.0
 * @apiGroup Occasions
 * @apiName Register route for admin
 * @apiDescription Route for registering an admin - An executive of MK2S_LLC
